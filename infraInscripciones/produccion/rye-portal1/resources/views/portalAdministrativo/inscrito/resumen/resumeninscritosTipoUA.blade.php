...
@section('content')
    <div class="card">
        ...
        <div class="card-body">
            <a href="{{ route('administrativo.resumeninscritos') }}" title="Back">
                <button class="btn btn-warning btn-sm col-md-3"><i class="fa fa-arrow-left" aria-hidden="true"></i>
                    Regresar
                </button>
            </a>
            <br>
            <form method="GET" action="{{ route('administrativo.resumeninscritosTipoUA',$tipo) }}" accept-charset="UTF-8" role="search">
                <div id="imprimible">
            ...
            <div class="table-responsive">
                ...
                <table class="table  table-striped table-bordered">
                    <thead>
                    <tr>
                        <th rowspan="2" class="text-center"><h5>RESUMEN {{isset($resumenInscritos[0]->descripcion)? strtoupper($resumenInscritos[0]->descripcion) : 'SIN DATOS'}}</h5></th>
                    ...
                    @foreach($resumenInscritos as $resumen)
                        <tr>
                            <th scope="row"><h5><a href="{{route('administrativo.resumeninscritosUA',[$resumen->tipo,$resumen->codfac])}}" class="badge badge-info">{{str_pad($resumen->codfac, 2, "0", STR_PAD_LEFT)}} - {{$resumen->nomcorto}}</a></h5></th>
