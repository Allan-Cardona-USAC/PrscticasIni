...
@section('content')
    <div class="card">
        ...
        <div class="card-body">
            <a href="{{route('administrativo.resumeninscritosUA',[$tipo,$ua]) }}" title="Back">
                <button class="btn btn-warning btn-sm col-md-3"><i class="fa fa-arrow-left" aria-hidden="true"></i>
                    Regresar
                </button>
            </a>
            <br>
            <form method="GET" action="{{ route('administrativo.resumeninscritosEXT',[$tipo,$ua,$ext]) }}" accept-charset="UTF-8" role="search">
                <div id="imprimible">
                ...
                <table class="table table-striped table-bordered">
                    ...
                    <tr>
                        <th rowspan="2" class="text-center">
                            <h5>U.A.: {{str_pad($ua, 2, "0", STR_PAD_LEFT)}} - {{isset($resumenInscritos[0]->nomcorto) ? $resumenInscritos[0]->nomcorto : 'SIN DATOS'}}
                                <br>EXT: {{str_pad($ext, 2, "0", STR_PAD_LEFT)}} - {{isset( $resumenInscritos[0]->nombre) ? $resumenInscritos[0]->nombre : 'SIN DATOS'}}
                            </h5>
                        </th>
                    ...
                    @foreach($resumenInscritos as $resumen)
                        <tr>
                            <th scope="row">
                                <h5>
                                    <a href="{{route('administrativo.resumeninscritosCARRERA',[$tipo,$ua,$ext,$resumen->codigo_carrera])}}" class="badge badge-info" >
                                        {{str_pad($resumen->codigo_carrera, 2, "0", STR_PAD_LEFT)}} -
                                        @php
                                            $nombre = explode("\n",wordwrap($resumen->nombre_carrera,25,"\n",false))
                                        @endphp
            ...
            <hr>
            <form method="POST" action="{{ route('administrativo.descargaCSV',[$ciclo, $semestre, $tipo, $ua, $ext, 0]) }}" accept-charset="UTF-8" class="form-horizontal" enctype="multipart/form-data" id="FormDatos">
                {{ csrf_field() }}
