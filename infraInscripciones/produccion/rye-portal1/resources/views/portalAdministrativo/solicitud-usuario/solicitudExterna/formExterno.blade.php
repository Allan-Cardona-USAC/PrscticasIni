<form method="POST" action="{{ route('administrativo.solicitarusuario') }}" accept-charset="UTF-8" class="form-horizontal" enctype="multipart/form-data" id="FormExterno" style="display: block;">
    {{ csrf_field() }}
    <div class="row justify-content-center">
        <div class="text-center mb-1">
            <h3><input class="text-center border-0 bg-transparent" value="Externo" readonly></h3>
        </div>
    </div>

    <div class="row justify-content-center">
        <div class="col-md-5 form-group {{ $errors->has('dependencia_iddependencia') ? 'has-error' : ''}}">
    ...
    <input hidden class="form-control form-control-sm bg-transparent border-0" style="color:transparent;" name="tipo" type="text" id="tipo" value="E" readonly>
    <div class="row">
    ...
    <div class="row">
        <div class="offset-md-2 col-md-4 form-group {{ $errors->has('copia_dpi') ? 'has-error' : ''}}">
            <label for="copia_dpi" class="control-label">{{ 'Copia Dpi (PDF)' }}</label>
            <input class="form-control" name="copia_dpi" type="file" id="copia_dpi" accept="application/pdf" value="{{ isset($solicitudusuario->copia_dpi) ? $solicitudusuario->copia_dpi : ''}}" required>
            {!! $errors->first('copia_dpi', '<p class="help-block">:message</p>') !!}
            <br/>
        </div>
        <div class="col-md-4 form-group {{ $errors->has('copia_oficio') ? 'has-error' : ''}}">
            <label for="copia_oficio" class="control-label">{{ 'Copia Oficio Solicitud Usuario (PDF)' }}</label>
            <input class="form-control" name="copia_oficio" type="file" id="copia_oficio" accept="application/pdf" value="{{ isset($solicitudusuario->copia_oficio) ? $solicitudusuario->copia_oficio : ''}}" required>
    ...
    <div class="form-group" align="center">
        ...
        {!! htmlFormSnippet() !!}
        <br/>
        <input class="btn btn-primary btn-block btn-sm col-md-4" type="submit" value="Crear">
    </div>
