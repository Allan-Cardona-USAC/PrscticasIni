<form method="POST" action="{{ url('/solicitud-usuario') }}" accept-charset="UTF-8" class="form-horizontal" enctype="multipart/form-data" id="FormInterno">
    {{ csrf_field() }}

    <div class="row justify-content-center">
        <div class="col-md-3 form-group {{ $errors->has('dependencia_iddependencia') ? 'has-error' : ''}}" id="internosDependencia">
            ...
            <select name="dependencia_iddependencia" class="form-control" id="dependencia_iddependencia">
                @foreach($dependenciasInterno as $dependencia )
                    <option value="{{ $dependencia->id }}">{{ $dependencia->id }} - {{$dependencia->descripcion}}</option>
                @endforeach
            </select>
        </div>
    </div>
    <input hidden class="form-control form-control-sm bg-transparent border-0" style="color:transparent;" name="tipo" type="text" id="tipo" value="I">
    <div class="row">
    ...
    <div class="row">
        ...
        <div class="col-md-3 form-group {{ $errors->has('registro_personal') ? 'has-error' : ''}}" id="registropersonal" >
            <label for="registro_personal" class="control-label">{{ 'Registro Personal' }}</label>
            <input class="form-control" name="registro_personal" type="number" id="registro_personal" value="{{ isset($solicitudusuario->registro_personal) ? $solicitudusuario->registro_personal : ''}}" required>
            {!! $errors->first('registro_personal', '<p class="help-block">:message</p>') !!}
        </div>
    </div>
