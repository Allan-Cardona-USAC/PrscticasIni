                        ...
                        <div class="card-body">
                            ...
                            <div class="row">
                                <div class="col-sm-12 col-12 col-lg-6">
                                    <button type="button" class="btn btn-danger btn-lg w-100" style="font-size:medium";  onClick="regresarEstado('{{$estudiante["id_orden_pago"]}}','{{$estudiante["carnet"]}}')"><i class="fa fa-undo"></i> Regresar Estado</button>
                                </div>
                                <div class="col-sm-12 col-12 col-lg-6">
                                    <button type="button" class="btn btn-primary mt-2 mt-md-0 btn-lg w-100 " style="font-size:medium"; onClick="imprimirCarnet('{{$estudiante["carnetmd5"]}}','{{$estudiante["id_orden_pago"]}}','{{$estudiante["carnet"]}}')"> <i class="fa fa-print"></i> <br>Imprimir</button>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <nav>
                <ul class="pagination flex-wrap">
                    <li class="page-item"><a class="page-link"
                            href="{{ url('administrativo/CarnetsGenerados?page=') . $previus }}">Previous</a></li>
                    @for ($i = 0; $i < $pages; $i++)
                        @if ($i + 1 <= $pages)
                            @if ($i + 1 == $page)
                                <li class="page-item active"><a class="page-link"
                                        href="{{ url('administrativo/CarnetsGenerados?page=') . ($i + 1) }}">{{ $i + 1 }} </a>
                                </li>
                            @else
                                <li class="page-item"><a class="page-link"
                                        href="{{ url('administrativo/CarnetsGenerados?page=') . ($i + 1) }}">{{ $i + 1 }}</a>
                    ...
                    @endfor
                    <li class="page-item"><a class="page-link"
                            href="{{ url('administrativo/CarnetsGenerados?page=') . $next }}">Next</a></li>
                </ul>
            </nav>
        </div>
    </div>
...
@section('javascript')
    <script>
        ...
        function regresarEstado(boleta, carnet) {
            console.log(carnet)
            data = {
                id: boleta,
                carnet: carnet
        ...
        function imprimirCarnet(carnetmd5,id,carnet) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) window.open(`https://registro.usac.edu.gt/imprimeCarne/ejecutarCarnet.php?PARAM_CARNET_IN=${carnet}&nombre=${carnetmd5}_firmado.pdf&currentUri=/reports/rye/CarnePVC`, '_blank');
